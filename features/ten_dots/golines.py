...
class LineDetectorHSV:
    ...
    def detectLines(self, color):
        ...
        cv2.imwrite('colorfilter.png', bw)
        cv2.imwrite('findedges.png', edge_color)
        lines = self._HoughLine(edge_color)
        centers, normals = self._findNormal(bw, lines)
        red = [0, 0, 255]
        # print(bw.shape)
        for x, y in centers:
            if randint(0, len(centers)) // 15 > 1:
                continue
            p = (int(x), int(y))
            cv2.circle(self.bgr, p, 3, red)

        cv2.imwrite('dots.png', self.bgr)
...
ldhsv = LineDetectorHSV()
ldhsv.setImage(cv2.imread('sample.png'))
ldhsv.detectLines('white')
